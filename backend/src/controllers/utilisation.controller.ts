...
export const getUtilisations = async (req: Request | any, res: Response) => {
  try {
    const { status, requisitionId } = req.query as any;
    const where: any = {};
...
export const getUtilisationSummary = async (req: Request | any, res: Response) => {
  try {
    const where: any = {};
    if (req.user.role === 'VERTICAL_USER') {
      where.vertical = req.user.vertical;
...
export const submitUtilisation = async (req: Request | any, res: Response) => {
  try {
    const data = req.body;
...
export const verifyUtilisation = async (req: Request | any, res: Response) => {
  try {
    const { id } = req.params;
    ...
    if (status === 'APPROVED' || status === 'VERIFIED') {
      if ((utilisation.amount || 0) > (utilisation.requisition?.releasedAmount || 0)) {

...
import { FileText, X, Eye, FileImage, ShieldAlert } from 'lucide-react';
...
const UtilisationPage: React.FC = () => {
  ...
  const [selectedUtil, setSelectedUtil] = useState<Utilisation | null>(null);
  ...
  const openReviewModal = (util: Utilisation) => {
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 rounded-t-xl overflow-x-auto">
          <div className="flex space-x-2 shrink-0">
            {['ALL', 'PENDING', 'VERIFIED', 'REJECTED'].map(status => (
              ...
              >
                {status === 'PENDING' ? 'SUBMITTED' : status.replace('VERIFIED', 'APPROVED')}
              </button>
            ))}
          </div>
        </div>

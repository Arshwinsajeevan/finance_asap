...
import { 
  ...
  Search, CheckCircle2, FileSpreadsheet, Building 
} from 'lucide-react';
...
interface EmployeeTaxRecord {
  ...
  pan: string;
  fy: string;
  status: 'READY' | 'PENDING_FINAL_PAYROLL';
}

// Mock Data for frontend visualization
const MOCK_GST_RECORDS: GstRecord[] = [
...
const MOCK_TDS_RECORDS: TdsRecord[] = [
  ...
  { id: 'TDS-002', vendor: 'Deep (Trainer)', pan: 'ZYXWV9876Q', section: '192', amountPaid: 65000, tdsDeducted: 6500, date: '2026-05-28', status: 'DEPOSITED' },
...
const MOCK_EMPLOYEES: EmployeeTaxRecord[] = [
...
const TaxationPage: React.FC = () => {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<'GST' | 'TDS' | 'FORM16'>('GST');

  const formatCurrency = (amount: number) => {
    return new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(amount);
  };

  const handleGenerateForm16 = (empName: string) => {
